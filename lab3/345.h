...
public:
    ...
    public:
        ...
        typeHeight getDelta() {
            auto hL = left == nullptr ? 0 : left->height;
            auto hR = right == nullptr ? 0 : right->height;
            return hR - hL;
        }
...
public:
    ...
    myBinaryTree<K, T>* insert(K key, T data) {
        ...
        if (key < head->key) {
            head->left = myBinaryTree<K, T>(head->left).insert(key, data)->head;   //TODO зачем тут класс???
        }
        else {
